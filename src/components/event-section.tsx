...
export default function EventSection({ event }: { event: Event }) {
  ...
  const isApproachingDeadline = !event.isCompleted && event.dueDate && daysUntilDue <= 5;

  if (isApproachingDeadline) {
    const clampedDays = Math.max(0, Math.min(daysUntilDue, 5));
    const urgencyProgress = 100 - (clampedDays * 16); 
    const hue = (clampedDays / 5) * 50;
    
    urgencyStyle = {
      padding: '2px',
      background: `conic-gradient(hsl(${hue}, 90%, 55%) ${urgencyProgress}%, transparent ${urgencyProgress}%)`,
    };
  }
  ...
  return (
    <Collapsible className="w-full">
       <div
        style={urgencyStyle}
        className={cn(isApproachingDeadline && "rounded-lg transition-all duration-300")}
       >
         <Card className={cn(
          "transition-all", 
